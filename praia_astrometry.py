...
def create_params_file(praia_header_output, user_catalog, catalog_code, output, idx, ccd_id):
    ...
    with open(os.path.join(os.getenv("APP_PATH"), "src/praia_astrometry.template.dat")) as template:
        ...
        if user_catalog not in file_catalogs:
            ...
            data = data.replace('{USER_CATALOG}', "user_catalog_placeholder.cat".ljust(50))
        ...
        ast_photometry = "%s/%s.ast_photometry.txt" % (os.getenv("DATA_DIR"), ccd_id)
...
def execute_astrometry(idx, header, catalog, catalog_code, logging):
    try:
        ...
        ccd_id = get_ccd_id_from_filepath(header.split('  ')[6])

        # Criar arquivo de input para cada execucao em paralelo.
        input_file = os.path.join(os.getenv("DATA_DIR"), "%s.astrometry_input.txt" % ccd_id)
        ...
        filename = os.path.join(os.getenv("DATA_DIR"), "%s.astrometry_params.dat" % ccd_id)
        params_file = create_params_file(input_file, catalog, catalog_code, filename, idx, ccd_id)

        # Exucao do praia astrometry
        praia_astrometry_output, output = run_praia_astrometry(idx, ccd_id, input_file, catalog, params_file)
...
def run_praia_astrometry(idx, ccd_id, praia_header_output, catalog,  params_file):
    ...
    exec_log = os.path.join(os.getenv("DATA_DIR"), "%s.praia_astrometry.log" % ccd_id)
    ...
    with open(exec_log, 'w') as fp:
        ...
        if process.returncode > 0:
            ...
            raise Exception("Failed to run PRAIA Astrometry. See log [%s] for more information" % exec_log)
